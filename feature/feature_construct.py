...
def act_features(data, mode):
    ...
    data = data.fillna(-1)
    for i in [3, 7]:
        temp = act.loc[act.day >= f-int(i)].groupby(['user_id']).size().reset_index().rename(columns={0: 'act_in_'+str(i)})
        data = pd.merge(data, temp, how='left', on=['user_id'])
    data = data.fillna(0)
    return data


def create_features(data, mode):
    if mode == 'train_mode':
        f = 24
    else:
        f = 31
    ...
    data = data.fillna(-1)
    for i in [3, 7]:
        temp = create.loc[create.day >= f-int(i)].groupby(['user_id']).size().reset_index().rename(columns={0: 'create_in_'+str(i)})
        data = pd.merge(data, temp, how='left', on=['user_id'])
    data = data.fillna(0)
    return data
...
if __name__ == '__main__':
    ...
    train = create_features(train, 'train_mode')
    train = act_features(train, 'train_mode')
    ...
    test = create_features(test, 'test_mode')
    test = act_features(test, 'test_mode')
