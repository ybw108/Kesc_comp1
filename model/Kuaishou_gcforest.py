...
def online(features):
    ...
    # gcf = gcForest(shape_1X=38, n_mgsRFtree=100, window=10, stride=2,
    ...
    gcf = gcForest(tolerance=0.0, n_cascadeRF=4, min_samples_cascade=20)
    _ = gcf.cascade_forest(train[features], train['label'])
    pred_proba = gcf.cascade_forest(X_test)
    tmp = np.mean(pred_proba, axis=0)
    test['predict'] = tmp[:, 1]
    test[['user_id', 'predict']].to_csv('../result/gcf_result.csv', index=False)


def online_MGC(features):
    X = train[features].fillna(-1)
    y = train['label']
    X = np.array(X)
    y = np.array(y)
    ...
    gcf = gcForest(shape_1X=38, n_mgsRFtree=100, window=19, stride=1,
                 cascade_test_size=0.2, n_cascadeRF=4, n_cascadeRFtree=101, cascade_layer=np.inf,
                 min_samples_mgs=0.1, min_samples_cascade=0.1, tolerance=0.0,)
    gcf.fit(X, y)
...
if __name__ == '__main__':
    ...
    result = pd.read_csv('../result/gcf_result.csv', index_col=False)
    result = result.sort_values(['predict'], ascending=False).head(25000)
    result['user_id'].to_csv('../result/gcf_sub.csv', index=False, header=False)
