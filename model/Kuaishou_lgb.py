...
def offline(features):
    ...
    for train_index, test_index in kf.split(X):
        ...
        model = gbm.fit(X_train, y_train, feature_name=features, eval_set=[(X_test, y_test)],
                        eval_metric='auc',
    ...
    print('average of best score: ' + str(sum(best_score)/len(best_score)))
...
def online(features):
    ...
    gbm = lgb.LGBMRegressor(objective='binary', seed=2018, n_estimators=56)

    gbm.fit(train[features], train['label'], feature_name=features)
    test['predicted_score'] = gbm.predict(test[features])

    test = test[test['predicted_score'] >= 0.7]
