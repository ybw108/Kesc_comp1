...
def offline(features):
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        #     for i in i_range:
        #         y_pred = (gbm.predict_proba(X_test, num_iteration=model.best_iteration_) >= i).astype(int)
        #         f1 = metrics.f1_score(y_test, y_pred)
        #         if f1 > best_f1:
        #             best_f1 = f1
        #             best_cutoff = i
        # else:
        #     y_pred = (gbm.predict_proba(X_test, num_iteration=model.best_iteration_) >= best_cutoff).astype(int)
        #     f1 = metrics.f1_score(y_test, y_pred)
        # f1_score.append(f1)
